...
已覆盖赛季、三档通行证、经验曲线、逐等级奖励、重复领取保护、任务类型和可配置 GUI。任务按周期稳定分配，完成后由 GUI 领取经验/奖励。菜单内部采用 DeluxeMenus 槽位模型，支持标准 `items:`、旧版顶层 slot 和常用 TrMenu 布局导入；游戏内编辑器使用会话 PDC 防止虚拟图标泄漏。更复杂的前置任务链、跨服排行榜和跨服交易补偿账本属于下一阶段。
